...
#cgo LDFLAGS: -L${SRCDIR}/bin -ldhnetsdk
...
extern int export_fSDKLogCallBack(char * szLogBuffer, u_int nLogSize, long dwUser);
...
func InitEx(cb CallBack_fDisConnect, stuInfo *NETSDK_INIT_PARAM) bool {
	...
	ret := C.CLIENT_InitEx(C.fDisConnect(C.export_fDisConnect), C.longlong(*userdata), (*C.struct_tagNETSDK_INIT_PARAM)(unsafe.Pointer(&lpInitParam)))
	if ret != 0 {
...
func Logout(loginhandle int) bool {
	...
	if C.CLIENT_Logout(C.longlong(loginhandle)) == 0 {
...
func LoginWithHighLevelSecurity(stInParam *NET_IN_LOGIN_WITH_HIGHLEVEL_SECURITY, stOutParam *NET_OUT_LOGIN_WITH_HIGHLEVEL_SECURITY) int {
	...
	(*lpInParmaInfo).dwSize = (C.DWORD)(unsafe.Sizeof(C.struct_tagNET_IN_LOGIN_WITH_HIGHLEVEL_SECURITY{}))
	...
	(*lPOutParmaInfo).dwSize = (C.DWORD)(unsafe.Sizeof(C.struct_tagNET_OUT_LOGIN_WITH_HIGHLEVEL_SECURITY{}))
...
func ListenServer(ip string, port uint16, nTimeout int, dwUserData ObjectId /*IF_fServiceCallBack*/) int {
	...
	ipTmp := C.CString(ip)
	ret := C.CLIENT_ListenServer(ipTmp, C.ushort(port), C.int(nTimeout), C.fServiceCallBack(C.export_fServiceCallBack), C.longlong(dwUserData))
	C.free(unsafe.Pointer(ipTmp))
	return int(ret)
}
...
func StopListenServer(lServerHandle int) bool {
	if C.CLIENT_StopListenServer(C.longlong(lServerHandle)) != 0 {
		return true
	}
	return false
}
...
func StartListenEx(lLoginID int) bool {
	...
	if C.CLIENT_StartListenEx(C.longlong(lLoginID)) != 0 {
		return true
	}
	return false
}
...
func StopListen(lLoginID int) bool {
	if C.CLIENT_StopListen(C.longlong(lLoginID)) != 0 {
		return true
	}
	return false
}
...
func SetDVRMessCallBack(dwUser ObjectId /*IF_fMessCallBack*/) {
	...
	C.CLIENT_SetDVRMessCallBack(C.fMessCallBack(C.export_fMessCallBack), C.longlong(dwUser))
}
...
func RealLoadPictureEx(lLoginID LLONG, nChannelID int32, dwAlarmType DWORD,
	...
	lAnalyzerHandle := C.CLIENT_RealLoadPictureEx(
		...
		C.longlong(*userdata),
		unsafe.Pointer(uintptr(unsafe.Pointer(nil))))
...
func StopLoadPic(lAnalyzerHandle LLONG) bool {
	...
	if C.CLIENT_StopLoadPic(C.longlong(lAnalyzerHandle)) != 0 {
...
func LogOpen(pstLogPrintInfo *LOG_SET_PRINT_INFO) bool {
	...
	lpInParmaInfo.dwSize = C.DWORD(unsafe.Sizeof(C.struct_tagLogSetPrintInfo{}))
...
func StartSearchDevicesEx(pInBuf *NET_IN_STARTSERACH_DEVICE, pOutBuf *NET_OUT_STARTSERACH_DEVICE) LLONG {
	...
	(*pstInBuf).dwSize = (C.DWORD)(unsafe.Sizeof(C.struct_tagNET_IN_STARTSERACH_DEVICE{}))

	var pstOutBuf *C.struct_tagNET_OUT_STARTSERACH_DEVICE = (*C.struct_tagNET_OUT_STARTSERACH_DEVICE)(unsafe.Pointer(pOutBuf))
	(*pstOutBuf).dwSize = (C.DWORD)(unsafe.Sizeof(C.struct_tagNET_OUT_STARTSERACH_DEVICE{}))
...
func StopSearchDevices(lSearchHandle LLONG) bool {
	...
	ret := C.CLIENT_StopSearchDevices(C.longlong(lSearchHandle))
	if ret == 0 {
		return false
	}
	return true
...
func SnapPictureEx(lLoginID int, par *SNAP_PARAMS, reserved unsafe.Pointer) bool {
	...
	ret := C.CLIENT_SnapPictureEx(C.longlong(lLoginID), parLocal, (*C.int)(reserved))
	if ret == 0 {
		return false
	}
	return true
...
func SetSnapRevCallBack(dwUser ObjectId /*IF_fSnapRev*/) {
	...
	C.CLIENT_SetSnapRevCallBack(C.fSnapRev(C.export_fSnapRev), C.longlong(dwUser))
}
...
func DownloadByTimeEx(lLoginID int, nChannelId int, nRecordFileType EM_QUERY_RECORD_TYPE, tmStart *NET_TIME, tmEnd *NET_TIME, sSavedFileName string, dwUserData ObjectId /*IF_fTimeDownLoadPosCallBack*/, dwDataUser ObjectId /*IF_fDataCallBack*/, pReserved unsafe.Pointer) int {
	...
	ret := C.CLIENT_DownloadByTimeEx(C.longlong(lLoginID), C.int(nChannelId), C.int(nRecordFileType), tmStartTmp, tmEndTmp, fileNameTmp,
		C.fTimeDownLoadPosCallBack(C.export_fTimeDownLoadPosCallBack), C.longlong(dwUserData), C.fDataCallBack(C.export_fDataCallBack), C.longlong(dwDataUser), pReserved)
...
func InitDevAccount(stInitAccountIn *NET_IN_INIT_DEVICE_ACCOUNT, stInitAccountOut *NET_OUT_INIT_DEVICE_ACCOUNT, dwWaitTime DWORD, szLocalIp string) bool {
	...
	(*pInitAccountIn).dwSize = (C.DWORD)(unsafe.Sizeof(C.struct_tagNET_IN_INIT_DEVICE_ACCOUNT{}))
	...
	(*pInitAccountOut).dwSize = (C.DWORD)(unsafe.Sizeof(C.struct_tagNET_OUT_INIT_DEVICE_ACCOUNT{}))

	szLocalIpTmp := C.CString(szLocalIp)
	ret := C.CLIENT_InitDevAccount(pInitAccountIn, pInitAccountOut, C.DWORD(dwWaitTime), szLocalIpTmp)
...
func SearchDevicesByIPs(stIpSearchInfo *DEVICE_IP_SEARCH_INFO, dwUserData IF_fSearchDevicesCB, szLocalIp string, dwWaitTime DWORD) BOOL {
	...
	(*pIpSearchInfo).dwSize = (C.DWORD)(unsafe.Sizeof(C.struct_tagDEVICE_IP_SEARCH_INFO{}))

	dwUserLocal := *(*int)(unsafe.Pointer(&dwUserData))

	var lSearchHandle C.int
	if len(szLocalIp) == 0 {
		lSearchHandle = C.CLIENT_SearchDevicesByIPs(pIpSearchInfo, C.fSearchDevicesCB(C.export_fSearchDevicesCB), C.longlong(dwUserLocal), nil, C.DWORD(dwWaitTime))
	} else {
		szLocalIpTmp := C.CString(szLocalIp)
		lSearchHandle = C.CLIENT_SearchDevicesByIPs(pIpSearchInfo, C.fSearchDevicesCB(C.export_fSearchDevicesCB), C.longlong(dwUserLocal), szLocalIpTmp, C.DWORD(dwWaitTime))
		C.free(unsafe.Pointer(szLocalIpTmp))
	}
...
func StopDownload(lFileHandle int) bool {
	if C.CLIENT_StopDownload(C.longlong(lFileHandle)) == 0 {
		return false
	}
	return true
}
...
func SetDeviceMode(lLoginID int, emType EM_USEDEV_MODE, pValue unsafe.Pointer) bool {
	if C.CLIENT_SetDeviceMode(C.longlong(lLoginID), C.EM_USEDEV_MODE(C.int(emType)), pValue) == 0 {
		return false
	}
	return true
}
...
func GetNMPDevInfo(lLoginID int, stInParam *NET_IN_GET_NMPDEV_INFO, stOutParam *NET_OUT_GET_NMPDEV_INFO, nWaitTime int32) bool {
	...
	(*lpInParmaInfo).dwSize = (C.DWORD)(unsafe.Sizeof(C.struct_tagNET_IN_GET_NMPDEV_INFO{}))

	lPOutParmaInfo := (*C.struct_tagNET_OUT_GET_NMPDEV_INFO)(unsafe.Pointer(stOutParam))
	(*lPOutParmaInfo).dwSize = (C.DWORD)(unsafe.Sizeof(C.struct_tagNET_OUT_GET_NMPDEV_INFO{}))

	ret := C.CLIENT_GetNMPDevInfo(C.longlong(lLoginID), lpInParmaInfo, lPOutParmaInfo, C.int(nWaitTime))
	if ret == 0 {
		return false
	}
	return true
